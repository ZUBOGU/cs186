    ...
    public void rollback(TransactionId tid)
        ...
        synchronized (Database.getBufferPool()) {
            synchronized(this) {
                ...
                raf.seek(offset);
                ArrayList<Page> befores = new ArrayList<Page>();
                while (true) {
                    try {
                        int cpType = raf.readInt();
                        long cpTid = raf.readLong();
                ...
                for (int i = befores.size(); i > 0; i--) {
                    Page p = befores.get(i - 1);
                    Catalog cat = Database.getCatalog();
                    DbFile db = cat.getDbFile(p.getId().getTableId());
                    db.writePage(p);
                }
            }
        }
    ...
    public void recover() throws IOException {
        synchronized (Database.getBufferPool()) {
            synchronized (this) {
                ...
                while (true) {
                    try {
                        ...
                        case CHECKPOINT_RECORD:
                            int numTransactions = raf.readInt();
                            while (numTransactions-- > 0) {
                                long loserid = raf.readLong();
                                long firstRecord = raf.readLong();
                                loserIds.put(loserid, firstRecord);
                            } 
                            raf.readLong();                
                            break;
                        case UPDATE_RECORD:
                            ...
                            buffer.replacePage(before.getId(), after);
                            raf.readLong();
                            break;
                        }
                    } catch (EOFException e) {
                ...
                // do all undo for loser transaction
                if (!loserIds.isEmpty()) {
                    undo(loserIds);
                }
        ...
        public void undo(HashMap<Long, Long> loserIds)
        throws NoSuchElementException, IOException {
        synchronized (Database.getBufferPool()) {
            synchronized(this) {
                // preAppend(); 
                // some code goes here
                Long min = Collections.min(loserIds.values());
                raf.seek(min);
                ArrayList<Page> befores = new ArrayList<Page>();
                while (true) {
                    try {
