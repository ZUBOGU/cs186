...
public class JoinOptimizer {
    ...
    public static int estimateTableJoinCardinality(Predicate.Op joinOp,
        ...
        // TODO: IMPLEMENT ME
        if (Predicate.Op.EQUALS.equals(joinOp) || Predicate.Op.LIKE.equals(joinOp)) {
            if (t1pkey && t2pkey) {
    ...
    public Vector<LogicalJoinNode> orderGreedyJoins(
        ...
        while(joinsLeft.size() > 0) {
            ...
            LogicalJoinNode bestjion = null;
            for (LogicalJoinNode join : joinsLeft) {
                CostCard greedyplan = costGreedyJoin(join ,plan, planCardinalities, planCosts, stats, filterSelectivities);
                if (bestplan == null || greedyplan.cost < bestplan.cost) {
                    bestplan = greedyplan;
                    bestjion = join;
                }
            }
            joinsLeft.remove(bestjion);
            plan.add(bestjion);
            planCardinalities.add(bestplan.card);
            planCosts.add(bestplan.cost);
        }
        return plan;
    }
    ...
    public Vector<LogicalJoinNode> orderDynamicProgrammingJoins(
        ...
        for (int i = 0; i < joins.size(); i++) {
            for (Set<LogicalJoinNode> s :  enumerateSubsets(joins, i + 1)) {
                ...
                for (LogicalJoinNode joinToRemove : s) {
                    CostCard plan = computeCostAndCardOfSubplan(stats, filterSelectivities, joinToRemove, s, bestCostSoFar, optjoin);
                    if (plan != null) {
                        bestplan = plan;
        ...
        return optjoin.getOrder(new HashSet<LogicalJoinNode>(joins)); //replace me
    }
