...
public class TableStats {
    ...
    private HashMap<Integer, IntStatistics> intType = new HashMap<Integer, IntStatistics>();
    private HashMap<Integer, StringHistogram> stringType = new HashMap<Integer, StringHistogram>();     
    ...
    public TableStats(int tableid, int ioCostPerPage) {
        ...
        // TODO: what goes here?
        int sizeOftuple = tupleDesc.numFields();
        for (int i = 0; i < sizeOftuple; i++) {
            if (tupleDesc.getFieldType(i) == Type.INT_TYPE) {
                intType.put(i, new IntStatistics(NUM_HIST_BINS));
            } else {
                stringType.put(i, new StringHistogram(NUM_HIST_BINS));
            }
        }
        ...
        try {
            ...
            while (iter.hasNext()) {
                ...
                // TODO: and here?
                for (int i = 0; i < sizeOftuple; i++) {
                    if (intType.containsKey(i)) {
                        intType.get(i).addValue(((IntField) t.getField(i)).getValue());
                    } else {
                        stringType.get(i).addValue(((StringField) t.getField(i)).getValue());
                    }
                }
            }
            iter.close();
    ...
    public double estimateSelectivity(int field, Predicate.Op op, Field constant) {
        // TODO: some code goes here
        if (intType.containsKey(field)) {
            return intType.get(field).estimateSelectivity(op, ((IntField) constant).getValue());
        } else {
            return stringType.get(field).estimateSelectivity(op, ((StringField) constant).getValue());
        }
    }
